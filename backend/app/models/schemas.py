...
class PartyInfo(BaseModel):
    """同行人数信息"""
    adults: int = Field(..., description="成人数量", ge=0, le=20)
    children: int = Field(default=0, description="儿童数量", ge=0, le=20)
    elders: int = Field(default=0, description="老人数量", ge=0, le=20)
    total: int = Field(..., description="同行总人数", ge=1, le=30)
    companion_type: str = Field(default="other", description="同行类型")
...
class BudgetConstraint(BaseModel):
    """预算约束"""
    amount: Optional[int] = Field(default=None, description="整趟旅行总预算；无明确预算时为null", ge=0)
    scope: str = Field(default="total", description="预算口径，固定为total")
    currency: str = Field(default="CNY", description="币种")
    budget_level: str = Field(default="standard", description="预算档位")
    strictness: str = Field(default="none", description="预算严格程度: hard/soft/none")
...
class TripPlanResponse(BaseModel):
    ...
    metadata: dict[str, Any] = Field(default_factory=dict, description="模型与工作流元数据")
