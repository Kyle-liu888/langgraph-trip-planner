<template>
  <article class="merchant-card" :data-kind="kind" :aria-label="`${merchant.name}门店资料`">
    <header class="merchant-heading">
      <div><span v-if="mealLabel" class="meal-label">{{ mealLabel }}</span><h3>{{ merchant.name }}</h3></div>
    ...
    <div class="merchant-body">
      ...
      <p v-if="hotelType || distance" class="merchant-note">{{ [hotelType, distance].filter(Boolean).join('；') }}（行程建议，未独立核验）</p>
      <p class="merchant-note">仅预算参考，非实时{{ kind === 'hotel' ? '房价或房态' : '菜单价格' }}；零估算不代表免费。</p>
      <p v-if="generic" class="merchant-status" role="status">这是{{ kind === 'hotel' ? '住宿' : '用餐' }}安排，尚未指定具体门店{{ kind === 'food' && merchant.name.includes('早餐') ? '；是否包含早餐请向住处确认' : '' }}。</p>
      <template v-else>
        <section class="merchant-facts" aria-label="门店来源资料">
          ...
          <p v-if="trustedRating"><span class="fact-label">高德参考评分</span><strong class="merchant-rating">{{ trustedRating }}</strong><span class="merchant-note">非实时评价，详见来源平台</span></p>
          <p v-else-if="info" class="merchant-note">暂无可核验评分</p>
          <p v-if="kind === 'food' && info?.source && info.food_tags.length"><span class="fact-label">高德餐饮特色</span>{{ info.food_tags.join('、') }}<span class="merchant-note">（非完整菜单）</span></p>
          <p v-if="status" class="merchant-status" role="status">{{ status }}</p>
          <p v-if="info?.source" class="merchant-note merchant-source">来源：{{ info.source.name }}<br>查询时间：{{ merchantQueryTime(info.source.queried_at) }}<br>上游更新时间：未提供</p>
        </section>
        <div v-if="photos.length" class="merchant-photos" aria-label="高德门店参考图片">
          <figure v-for="(photo, index) in photos" :key="photo.url">
            ...
            <figcaption>门店参考图<span v-if="photo.title && photo.title !== '门店参考图'">：{{ photo.title }}</span></figcaption>
          </figure>
        </div>
        <p v-if="photos.length" class="merchant-note">图片来源：高德地图；未独立核验拍摄时间和具体房型／菜品。</p>
        ...
        <section class="merchant-links" aria-label="平台门店入口">
          ...
          <p v-if="links.length" class="merchant-note">人工核验日期：{{ links.map(link => `${platformNames[link.platform]} ${link.verified_at}`).join('；') }}。非实时核验，链接可能变化。</p>
          <p v-if="amapUrl || links.length" class="merchant-note">评价与更多图片请在平台查看；目标页面不保证有评价。</p>
        </section>
        <section class="merchant-search" aria-label="小红书搜索入口">
          ...
          <p class="merchant-note search-keyword">搜索词：{{ search.keyword }}</p>
          <p v-if="copyMessage" role="status" class="merchant-note">{{ copyMessage }}</p>
          <p class="merchant-note">这是搜索入口，不是已核验评价；平台可能要求登录。入口失效时，可复制搜索词到小红书查询。</p>
        </section>
      </template>
    </div>
...
import { isGenericMerchant, merchantKey, merchantQueryTime, merchantSearch, merchantStatus, safeMerchantLink,
...
const status = computed(() => merchantStatus(props.info))
...
watch(() => merchantKey(props.city, { kind: props.kind, merchant: props.merchant }), () => {
  failedPhotos.value = new Set(); copyMessage.value = ''; previewDialog.value?.close(); selectedPhoto.value = undefined
})
...
async function copyKeyword() {
  try { await navigator.clipboard.writeText(search.value.keyword); copyMessage.value = '搜索词已复制' }
  catch { copyMessage.value = '复制失败，请选中上方搜索词手动复制' }
}
</script>
...
.meal-label { display: inline-block; margin-right: 10px; color: var(--color-primary, #176b5b); font-size: 12px; border-bottom: 2px solid #aac9aa; vertical-align: 2px; }
...
.merchant-source { margin-top: 10px !important; }
...
.merchant-photos figcaption { font-size: 11px; color: var(--color-muted, #657873); margin-top: 4px; }
...
@media (max-width: 600px) {
  ...
  .merchant-photos figcaption { font-size: 10px; }
}
</style>
