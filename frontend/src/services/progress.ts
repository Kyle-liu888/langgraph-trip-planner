...
class RefreshSession extends Error {}
...
export async function subscribeProgress(id: string, signal: AbortSignal,
  ...
  let cursor = 0, retries = 0, refreshed = false
  while (!signal.aborted) {
    ...
    try {
      const token = await accessToken().catch(() => { throw new FatalStreamError('登录已失效，请重新登录') })
      ...
      await fetchEventSource(`${API_BASE_URL}/api/trips/${id}/events`, {
        ...
        async onopen(response) {
          ...
          if (response.status === 401) throw new RefreshSession()
          ...
          retries = 0; refreshed = false; onStatus('实时连接已建立')
        },
        onmessage(message) {
      ...
      if (refresh) throw new RefreshSession()
      if (timedOut) throw new Error('进度心跳超时')
    } catch (error) {
      if (signal.aborted) return
      if (error instanceof FatalStreamError) throw error
      if (error instanceof RefreshSession) {
        if (refreshed) throw new Error('登录令牌校验失败，请重新登录；检查后端 Supabase 地址与签名配置')
        const { data, error: authError } = await supabase!.auth.refreshSession()
        if (authError || !data.session) throw new Error('登录已过期，请重新登录')
        refreshed = true
      }
      retries++
